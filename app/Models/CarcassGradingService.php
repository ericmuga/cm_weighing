 ...
 *     (Premium=5 .. Commercial=1) and is the tie-breaker, but per the sheet
 *     ("Weight cannot improve the grading") it can only ever pull the final
 *     grade down to what the weight tier supports, never up.
 *   - Weight is only actually needed for that tie-break. When verdict1 lands
 *     in exactly one grade's band, compute() resolves it without weight;
 *     weight is required only once verdict1 ties between multiple bands, and
 *     compute() returns with classification still null (candidates lists the
 *     tie) until it's supplied.
 */
class CarcassGradingService
{
    ...
    public static function compute(array $attributes, ?float $weight): array
    {
        ...
        // bands (see class docblock) — an unambiguous verdict1 resolves on
        // its own. So it's only fetched/applied from here on.
        ...
        if (count($candidates) === 1) {
            $grade = $candidates[0];
            // Weight can only ever pull the grade down, never up — and only
            // once it's actually known.
            if ($hasWeight && $weightTier < self::GRADE_BANDS[$grade]['tier']) {
        ...
        // Tied bands: weight is the only thing that can break the tie — if it
        // isn't known yet, leave classification null (candidates still lists
        // the tie) rather than guessing.
        if (!$hasWeight) {
            return $result;
        }

        // ...and only downward — pick the best one the actual weight tier
        // still supports.
        ...
        // Weight didn't clearly support any of the tied candidates either —
        // settle on the lowest tied grade and flag the row for QA review.
        $ranked = $candidates;
        usort($ranked, fn ($a, $b) => self::GRADE_BANDS[$a]['tier'] <=> self::GRADE_BANDS[$b]['tier']);
